...
{
  public ValueTask<ShaderProgramDeclaration> ParseShaderAsync(string path, TextReader reader, int length, CancellationToken cancellationToken = default)
  {
    ...
    return ValueTask.FromResult(new ShaderProgramDeclaration(path, ShaderArchetype.Sprite, compilationUnit));
  }
}
