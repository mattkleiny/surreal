...
namespace Surreal.IO
{
  public static class PathExtensions
  {
    ...
    public static async ValueTask<bool> ExistsAsync(this Path path)      => await path.GetFileSystem().ExistsAsync(path.Target);
    public static async ValueTask<bool> IsFileAsync(this Path path)      => await path.GetFileSystem().IsFileAsync(path.Target);
    public static async ValueTask<bool> IsDirectoryAsync(this Path path) => await path.GetFileSystem().IsDirectoryAsync(path.Target);
    ...
    public static ValueTask<Path[]> EnumerateAsync(this Path path, string wildcard)
    {
      return path.GetFileSystem().EnumerateAsync(path.Target, wildcard);
    }

    public static async ValueTask<Size> GetSizeAsync(this Path path)
    {
      return await path.GetFileSystem().GetSizeAsync(path.Target);
    }

    public static async ValueTask<Stream> OpenInputStreamAsync(this Path path, int bufferSize = DefaultBufferSize)
    {
    ...
    public static async ValueTask<Stream> OpenOutputStreamAsync(this Path path, int bufferSize = DefaultBufferSize)
    {
