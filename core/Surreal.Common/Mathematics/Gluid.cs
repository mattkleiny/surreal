...
/// <summary>
/// A special representation of a Guid that can contain (or internalise) one or two 4-byte integers or a single long
/// in a reversible way against a namespace.
/// <para/>
...
/// </summary>
/// <remarks>
/// Originally from https://github.com/b-hulme/gluid/tree/master, but vendored here to avoid a dependency.
/// </remarks>
public static class Gluid
{
  ...
  {
    return NewGluid(value, 0, @namespace);
  }
  ...
  public static int? ToInt32(this Guid guid, string? @namespace = null) =>
      TryTranscodeAndCheck(guid, @namespace, out var guidBytes) ? BitConverter.ToInt32(GetLowerBytes(guidBytes), 0) : null;

  public static long? ToInt64(this Guid guid, string? @namespace = null) =>
      TryTranscodeAndCheck(guid, @namespace, out var guidBytes) ? GetInt64(guidBytes) : null;

  public static int? GetSecondInt32(this Guid guid, string? @namespace = null) =>
      TryTranscodeAndCheck(guid, @namespace, out var guidBytes) ? BitConverter.ToInt32(GetUpperBytes(guidBytes), 0) : null;
  ...
  private static byte GetVersion(byte[] guidBytes) => (byte)(guidBytes[GuidVersionByteNumber] & GuidVersionMask);
  private static byte GetVariant(byte[] guidBytes) => (byte)(guidBytes[GuidVariantByteNumber] & GuidVariantMask);
  ...
  private static byte[] GetHash(string @namespace)
  {
    ...
    var hashArray = SHA256.HashData(Encoding.UTF8.GetBytes(@namespace));
