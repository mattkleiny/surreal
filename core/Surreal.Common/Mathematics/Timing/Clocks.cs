...
namespace Surreal.Mathematics.Timing {
  ...
  public static class Clocks {
    ...
    public static async Task EvaluateOverTime(this IClock clock, TimeSpan duration, Action<Normal> action, CancellationToken cancellationToken = default) {
      var currentTime  = 0f;
      ...
      while (currentTime <= totalSeconds && !cancellationToken.IsCancellationRequested) {
        ...
        action(currentTime / totalSeconds);
