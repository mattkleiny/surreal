...
/// <summary>
/// A <see cref="IScriptParser"/> for Lisp programs.
/// </summary>
public sealed class LispScriptParser : IScriptParser
{
  ...
  private sealed class LispParserContext(IEnumerable<SymbolicExpression> expressions)
  {
    ...
    public CompilationUnit ParseCompilationUnit()
    {
      ...
      while (_expressions.TryPeek(out _))
      {
    ...
    {
      if (_expressions.TryDequeue(out var expression))
      {
        switch (expression)
        {
  ...
  /// <summary>
  /// A symbolic expression (or s-expression), for use in recursive deconstruction of the LISP tree.
  /// </summary>
