...
public abstract class GameActor : Actor
{
  ...
  public Rectangle Bounds => FinalPolygon.Bounds.Clamp(0, 0, _canvas.Width - 1, _canvas.Height - 1);
  ...
  private void CheckIfOffScreen()
  {
    ...
    {
      Position.X = _canvas.Width + halfSize.X; // left
    }

    if (Velocity.Y < 0f && Position.Y - halfSize.Y < 0)
    {
      Position.Y = _canvas.Height - halfSize.Y; // top
    }

    if (Velocity.X > 0f && Position.X - halfSize.X > _canvas.Width)
    {
      Position.X = -halfSize.X; // right
    }

    if (Velocity.Y > 0f && Position.Y + halfSize.Y > _canvas.Height)
    {
  ...
  private void DrawPolygon()
  {
    ...
    for (var y = (int) rectangle.Bottom; y < (int) rectangle.Top; y++)
    for (var x = (int) rectangle.Left; x < (int) rectangle.Right; x++)
    {
