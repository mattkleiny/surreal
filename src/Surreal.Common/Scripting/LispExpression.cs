    ...
    switch (root.Contents.Dequeue())
    {
      ...
      case SExpression.Atom atom:
      {
        switch (atom.Value)
        {
          ...
          case "lambda":
            Debug.Assert(root.Contents.Peek() is SExpression.List,
              "\"lambda\" statements should be followed by list of parameters");

            var parameters = (SExpression.List) root.Contents.Dequeue();
            Debug.Assert(parameters.Contents.All(x => x is SExpression.Atom),
          ...
          case "define":
            Debug.Assert(root.Contents.Peek() is SExpression.Atom,
